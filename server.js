...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === '/proxy') {
    ...
    let decodedUrl;
    try { decodedUrl = decodeURIComponent(targetUrl); } catch(e) { res.writeHead(400); res.end('invalid url'); return; }
    ...
    try {
      ...
      const isM3u8 = decodedUrl.includes('.m3u8') || (resp.headers['content-type'] || '').includes('mpegurl');
      const isTs   = decodedUrl.includes('.ts')   || (resp.headers['content-type'] || '').includes('mp2t');
      ...
      if (isM3u8) {
        ...
        headers['Content-Type'] = resp.headers['content-type'] || 'application/octet-stream';
        res.writeHead(resp.status || 200, headers);
        res.end(resp.buffer);
      }
    } catch(err) {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    res.writeHead(200, Object.assign({}, corsHeaders(), { 'Content-Type': MIME[ext] || 'text/plain' }));
    res.end(data);
  });
});
